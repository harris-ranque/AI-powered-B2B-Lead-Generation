...
import { TrendingUp, Users, Mail, Target, Clock, Search, Bot, AlertTriangle, Bell, X, CheckCircle, Bug } from "lucide-react";
...
export function Dashboard() {
  ...
  const recentCompletedSearches = searches?.filter(s => 
    s.status === 'completed' && 
  ...
  const visibleUrgentBroadcasts = urgentBroadcasts.filter(b => 
    !dismissedAlerts.has(b._id)
  );
          ...
          {visibleUrgentBroadcasts.length > 0 && (
            <div className="mb-6 space-y-3">
              {visibleUrgentBroadcasts.map((broadcast) => {
                ...
                return (
                  <Alert key={broadcast._id} className={`${priorityDisplay.bgColor} border-l-4`}>
                        ...
                        {broadcast.data && (
                          <div className="text-xs bg-background/50 p-2 rounded border">
                            {broadcast.data.creditBalance !== undefined && (
                              <span>Credit Balance: {broadcast.data.creditBalance}</span>
                            )}
                            {broadcast.data.searchId && (
                              <span>Search ID: {broadcast.data.searchId}</span>
                            )}
                            {broadcast.data.rateLimitType && (
                              <span>Rate Limit: {broadcast.data.rateLimitType}</span>
                            )}
                          </div>
                        )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="p-6 bg-card border-border">
              <div className="flex items-center justify-between mb-4">
                ...
                {(searchBroadcasts.length > 0 || creditBroadcasts.length > 0) && (
              ...
              <div className="space-y-4">
                ...
                {searchBroadcasts.slice(0, 2).map((broadcast) => {
                  const priorityDisplay = getPriorityDisplay(broadcast.priority);
                  return (
                    <div key={broadcast._id} className="flex items-start gap-2 p-2 bg-muted/30 rounded-lg">
          ...
          {rateLimitWarnings.length > 0 && (
            <Card className="p-4 bg-yellow-50 border-yellow-200 mt-6">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h4 className="font-semibold text-yellow-800 mb-2">Rate Limit Notifications</h4>
                  <div className="space-y-2">
                    {rateLimitWarnings.slice(0, 3).map((warning) => (
                      <div key={warning._id} className="text-sm text-yellow-700">
                ...
                {recentCompletedSearches.slice(0, 3).map((search) => (
                  <div key={search._id} className="p-3 border rounded-lg">
                    ...
                    <div className="text-xs text-muted-foreground space-y-1">
                      ...
                      <p>Completed: {formatBroadcastTime(search.completedAt)}</p>
                    </div>
                  </div>
                ))}
              </div>
