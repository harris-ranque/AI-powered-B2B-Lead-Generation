...
export function validateEnvironment(): ValidationResult {
  ...
  for (const varName of OPTIONAL_VARS) {
    const value = env[varName];
    if (value && (value.includes('placeholder') || value.includes('...') || value.includes('your_'))) {
...
export function getEnvironmentInfo() {
  ...
  return {
    ...
    base: env.BASE_URL,
    validation,
    convexConfigured: Boolean(env.VITE_CONVEX_URL && env.VITE_CONVEX_URL !== 'https://placeholder.convex.cloud'),
    hasStripe: Boolean(env.VITE_STRIPE_PUBLISHABLE_KEY && !env.VITE_STRIPE_PUBLISHABLE_KEY.includes('...')),
    hasAnalytics: Boolean(env.VITE_POSTHOG_KEY && !env.VITE_POSTHOG_KEY.includes('...')),
  };
}
