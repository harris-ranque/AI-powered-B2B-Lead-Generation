...
export function useCorrelationLogs() {
  ...
  const buildCorrelationTree = useCallback(
    (logs: CorrelationLog[]): CorrelationTree[] => {
      ...
      const buildNode = (correlationId: string): CorrelationTree | null => {
        ...
        const hasRunning = nodeLogs.some(
          (log) =>
            log.message.includes("started") ||
            log.message.includes("processing"),
        );
        const hasCompleted = nodeLogs.some(
          (log) =>
            log.message.includes("completed") ||
            log.message.includes("finished"),
        );

        const status: CorrelationTree["status"] = hasError
          ? "failed"
          : hasCompleted
            ? "completed"
            : "running";
        const level: CorrelationTree["level"] = hasError
          ? "error"
          : hasWarn
            ? "warn"
            : "info";
      ...
      const rootIds = Array.from(correlationMap.keys()).filter(
        (id) => !parentMap.has(id),
      );
  ...
  const getSearchLogs = useCallback(
    (searchId: Id<"searches">) => {
  ...
  const performanceLogs =
    logs?.filter((log) => log.metadata.duration !== undefined) || [];
...
export function useCorrelationTracker(correlationId?: string) {
  ...
  const correlationTree = correlationTrees.find(
    (tree) => tree.correlationId === correlationId,
  );

  // Get all logs for this correlation (including children)
  const correlationLogs = logs.filter(
    (log) =>
...
function calculateProgress(tree: CorrelationTree): number {
  const calculateNodeProgress = (node: CorrelationTree): number => {
    ...
    const completedSteps = node.logs.filter(
      (log) =>
        log.message.includes("completed") ||
        log.message.includes("finished") ||
  ...
  const totalProgress = childProgresses.reduce(
