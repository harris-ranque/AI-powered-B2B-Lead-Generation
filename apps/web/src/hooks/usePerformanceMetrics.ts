...
export function usePerformanceMetrics() {
  ...
  const searchMetrics = useMemo(() => {
    ...
    const completedSearches = searches.searches.filter(
      (s) => s.status === "completed",
    ).length;
    const failedSearches = searches.searches.filter(
      (s) => s.status === "failed",
    ).length;
    ...
    const averageLeadsPerSearch =
      completedSearches > 0 ? totalLeadsGenerated / completedSearches : 0;
  ...
  const systemMetrics = useMemo(() => {
    ...
    const recentLogs = logs.filter(
      (log) => Date.now() - log._creationTime < 60 * 60 * 1000,
    ); // Last hour
  ...
  const pipelineMetrics = useMemo(() => {
    ...
    const calculateStageMetrics = (stage: string) => {
      ...
      const completedOperations = stageLogs.filter(
        (log) =>
  ...
  const creditMetrics = useMemo(() => {
    ...
    const averageCostPerSearch =
      completedSearches > 0 ? totalCreditsUsed / completedSearches : 0;

    // Calculate today's usage
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const todaySearches = searches.searches.filter(
      (s) => s._creationTime >= today.getTime() && s.status === "completed",
    );
    const creditsUsedToday = todaySearches.reduce(
  ...
  useEffect(() => {
    const updateRealTimeMetrics = () => {
      ...
      const activeSearches = searches.searches.filter(
      ...
      const recentErrors = logs.filter(
        (log) =>
          log.level === "error" &&
          Date.now() - log._creationTime < 5 * 60 * 1000,
      ).length;

      const errorRate =
        logs.length > 0 ? (recentErrors / Math.min(logs.length, 100)) * 100 : 0;
