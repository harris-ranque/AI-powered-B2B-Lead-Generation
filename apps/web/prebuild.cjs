// Pre-build script to bundle shared types for Railway deployment
const fs = require('fs');
const path = require('path');

// Create a local copy of shared types for Railway deployment
const sharedTypesSource = path.join(__dirname, '../../packages/shared-types');
const sharedTypesDest = path.join(__dirname, 'src/shared-types-local');

// Remove old directory if exists
if (fs.existsSync(sharedTypesDest)) {
  fs.rmSync(sharedTypesDest, { recursive: true });
}

// Create directory
fs.mkdirSync(sharedTypesDest, { recursive: true });

// Always create complete stub files for Railway deployment since it's isolated
console.log('Creating shared types for Railway deployment');

const royaltyTypesContent = `// Developer configuration and payout settings
...
const indexContent = `export * from './royalty.types';
`;

fs.writeFileSync(path.join(sharedTypesDest, 'royalty.types.ts'), royaltyTypesContent);
fs.writeFileSync(path.join(sharedTypesDest, 'index.ts'), indexContent);

console.log('✓ Shared types created successfully for Railway deployment');