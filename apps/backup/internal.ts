...
export const batchAnalyzeLeads: any = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const leadsToAnalyze: any = leads.filter(
        (l: any) =>
      ...
      console.log(
        `Starting batch AI analysis for ${leadsToAnalyze.length} leads`,
      );
      ...
      for (let i = 0; i < leadsToAnalyze.length; i += batchSize) {
        const batch = leadsToAnalyze.slice(
          i,
          Math.min(i + batchSize, leadsToAnalyze.length),
        );
      ...
      console.log(
        `Batch AI analysis completed: ${analyzedCount} analyzed, ${failedCount} failed`,
      );
