...
export const orchestrateSearch: any = internalAction({
  args: {
    searchId: v.id("searches"),
    startFromStage: v.optional(
      v.union(
        v.literal("discovery"),
  ...
  handler: async (ctx, args) => {
    ...
    const searchRecord = await ctx.runQuery(
    ...
    try {
      ...
      if (currentStage === "discovery") {
        try {
          ...
          const discoveryResult = await ctx.runAction(
            api.search.actions.searchGoogleMaps,
            {
              searchId,
            },
          );
      ...
      if (currentStage === "enrichment" && pipelineSuccess) {
        try {
          ...
          const enrichmentResult = await ctx.runAction(
            internal.leads.enrichment.batchEnrichLeads,
            {
              searchId,
              correlationId,
            },
      ...
      if (currentStage === "analysis" && pipelineSuccess) {
        try {
          ...
          const user = await ctx.runQuery(
            internal.users.queries.getUserInternal,
            {
              userId: searchRecord.userId,
            },
          );
          ...
          if (user && (user.plan === "pro" || user.plan === "enterprise")) {
            ...
            const analysisResult = await ctx.runAction(
              internal.langgraph.internal.batchAnalyzeLeads,
              {
                searchId,
                correlationId,
              },
...
export const resumeSearch: any = internalAction({
  args: {
    searchId: v.id("searches"),
  },
  handler: async (ctx, args) => {
    ...
    const leads = await ctx.runQuery(
      internal.leads.queries.getSearchLeadsInternal,
      {
        searchId: args.searchId,
      },
    );
...
export const checkStuckSearches: any = internalAction({
  args: {},
  handler: async (ctx) => {
    ...
    const stuckSearches: any = await ctx.runQuery(
      internal.search.internal.getStuckSearches,
      {
        stuckThreshold,
      },
    );
    ...
    for (const search of stuckSearches) {
      try {
        ...
        const result = await ctx.runAction(
          internal.search.orchestrator.resumeSearch,
          { searchId: search._id },
        );
