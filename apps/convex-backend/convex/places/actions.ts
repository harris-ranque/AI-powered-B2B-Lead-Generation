...
export const getPlacePredictions = action({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const types = args.types && args.types.length > 0 ? args.types : ["(cities)"];
      if (types.length > 0) {
      ...
      return {
        success: false,
        predictions: [],
        error: error instanceof Error ? error.message : "Failed to fetch location suggestions",
      };
    }
  },
});
...
export const getPlaceDetails = action({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const fields = args.fields && args.fields.length > 0 ? args.fields : [
        "place_id",
        "formatted_address", 
        "geometry",
        "name",
      ...
      if (!response.ok) {
        throw new Error(`Google Places Details API HTTP error: ${response.status} ${response.statusText}`);
      }
