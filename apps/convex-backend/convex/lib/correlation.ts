...
export function endPerformanceTracking(start: {
  startTime: number;
  ...
  return Promise.race([
    ...
    new Promise<never>((_, reject) => {
      setTimeout(() => {
        ...
        reject(
          enhanceError(timeoutError, correlation, { timeoutMs, operationName }),
        );
      }, timeoutMs);
    }),
  ]);
...
export function formatCorrelationForLogging(
  correlation: CorrelationContext,
): Record<string, any> {
  const formatted = {
    ...
    ...(correlation.metadata && {
      ...
      ...(correlation.metadata.environment && {
        environment: correlation.metadata.environment,
      }),
      ...(correlation.metadata.version && {
        version: correlation.metadata.version,
      }),
      ...(correlation.metadata.feature && {
        feature: correlation.metadata.feature,
      }),
    }),
  };
