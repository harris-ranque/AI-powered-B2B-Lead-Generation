  ...
  handler: async (ctx) => {
    ...
    try {
      ...
      const emailRequests = allRequests.filter(
        (req) => req.type === "email_generation",
      );

      const successfulEmails = emailRequests.filter(
        (req) => req.status === "completed",
      );
      const failedEmails = emailRequests.filter(
        (req) => req.status === "failed",
      );

      const recentEmails = emailRequests.filter(
        (req) => req.createdAt > sevenDaysAgo,
      );
      const monthlyEmails = emailRequests.filter(
        (req) => req.createdAt > thirtyDaysAgo,
      );
      ...
      const completedWithTiming = successfulEmails.filter(
        (req) => req.startedAt && req.completedAt,
      );
        ...
        completedWithTiming.length > 0
          ? Math.round(
              completedWithTiming.reduce(
                (sum, req) =>
                  sum + ((req.completedAt || 0) - (req.startedAt || 0)),
                0,
      ...
      const emailTypes = emailRequests.reduce(
        (acc, req) => {
...
export const getAgentPerformanceStats = query({
  ...
  handler: async (ctx) => {
    ...
    try {
      ...
      const agentStats = requests.reduce(
        (acc, req) => {
