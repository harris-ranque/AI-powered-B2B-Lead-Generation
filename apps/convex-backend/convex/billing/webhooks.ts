...
export const handleSubscriptionUpdate = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const customerBilling = await ctx.db
        .query("billing")
        .withIndex("by_stripe_customer", (q) => q.eq("stripeCustomerId", subscription.customer))
        .unique();

      if (!customerBilling) {
        console.error(`Billing record not found for customer ID: ${subscription.customer}`);
        return;
      }

      // Get the user
      const user = await ctx.db.get(customerBilling.userId);
      if (!user) {
        console.error(`User not found for user ID: ${customerBilling.userId}`);
        return;
      }

      // Get or create existing billing record for subscription
      let existingBilling = await ctx.db
        .query("billing")
      ...
      if (existingBilling) {
        ...
        await ctx.db.patch(existingBilling._id, subscriptionData);
      } else {
...
export const handlePaymentSuccess = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const billing = await ctx.db
        .query("billing")
        .withIndex("by_stripe_customer", (q) => q.eq("stripeCustomerId", invoice.customer))
        .unique();

      if (!billing) {
        console.error(`Billing record not found for customer ID: ${invoice.customer}`);
        return;
      }
      ...
      if (!user) {
        console.error(`User not found for user ID: ${billing.userId}`);
        return;
      }
