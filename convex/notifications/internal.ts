  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      byType: {
        ...
        credit_alert: notifications.filter(n => n.type === "credit_alert").length,
        search_completed: notifications.filter(n => n.type === "search_completed").length,
        plan_updated: notifications.filter(n => n.type === "plan_updated").length,
        weekly_summary: notifications.filter(n => n.type === "weekly_summary").length,
      },
    };
  },
});