...
Lead Eternity is a sophisticated AI-powered lead generation platform built as a monorepo using Turbo. The system combines a React frontend with a Python CrewAI worker service to generate personalized email sequences for business leads.
...
Create `lead-eternity-convex/.env.local`:
```env
# API Keys
OPENAI_API_KEY=sk-...
GOOGLE_MAPS_API_KEY=...
...
- **Backend**: Convex deployed separately from `lead-eternity-convex` repository
